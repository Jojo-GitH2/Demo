...
# # Do not modify the running script in-place. Set NGINX_CONF at runtime instead.
# # NGINX_CONF will be defined inside the remote heredoc where it's needed.
# # === SSH connectivity dry-run ===
# printf 'Checking SSH connectivity to %s@%s ...\n' "$REMOTE_USER" "$REMOTE_HOST" | tee -a "$LOGFILE"
# # quick TCP check for port 22 (may fail if firewall blocks ICMP but useful)
...
# # Non-interactive SSH test (BatchMode) to verify key/auth works
# if ssh -o BatchMode=yes -o ConnectTimeout=5 -i "$SSH_KEY" "${REMOTE_USER}@${REMOTE_HOST}" 'echo SSH_OK' >/dev/null 2>&1; then
#   printf 'SSH key auth ok (non-interactive)\n' | tee -a "$LOGFILE"
# else
#   printf 'Non-interactive SSH auth failed — trying interactive SSH (may prompt)...\n' | tee -a "$LOGFILE"
#   if ssh -o ConnectTimeout=10 -i "$SSH_KEY" "${REMOTE_USER}@${REMOTE_HOST}" 'echo SSH_OK' >>"$LOGFILE" 2>&1; then
...
# # Create remote directory structure (no-sudo directory under user's home)
...
# # Prepare remote environment: install Docker, docker-compose plugin and nginx if possible.
# # This attempts passwordless sudo (sudo -n). If sudo requires a password the step will fail and instruct the user.
...
# set -e
# # If docker missing, install Docker CE and the compose plugin from Docker's official repo
...
#     sudo apt-get update -y
#     sudo apt-get install -y docker-ce docker-ce-cli containerd.io docker-compose-plugin nginx
#     sudo systemctl enable --now docker nginx || true
#     sudo usermod -aG docker "${REMOTE_USER}" || true
#   else
#     echo "Remote sudo without password is required to auto-install docker/nginx. Please run on the remote host:" >&2
#     echo "  sudo apt-get update && sudo apt-get install -y ca-certificates curl gnupg lsb-release apt-transport-https" >&2
#     echo "  sudo mkdir -p /etc/apt/keyrings && curl -fsSL https://download.docker.com/linux/ubuntu/gpg | sudo gpg --dearmor -o /etc/apt/keyrings/docker.gpg" >&2
#     echo "  echo \"deb [arch=\$(dpkg --print-architecture) signed-by=/etc/apt/keyrings/docker.gpg] https://download.docker.com/linux/ubuntu \$(lsb_release -cs) stable\" | sudo tee /etc/apt/sources.list.d/docker.list" >&2
#     echo "  sudo apt-get update && sudo apt-get install -y docker-ce docker-ce-cli containerd.io docker-compose-plugin nginx" >&2
#     echo "  sudo systemctl enable --now docker nginx" >&2
#     echo "  sudo usermod -aG docker ${REMOTE_USER}" >&2
#     exit 2
#   fi
# fi

# # Report versions (best-effort)
# docker --version || true
# docker compose version || true
# nginx -v || true
# EOF
# if [ $? -ne 0 ]; then
#   printf 'Remote environment preparation failed (see log). Aborting.\n' | tee -a "$LOGFILE"
#   exit $E_REMOTE_FAIL
# fi

# # Transfer files (use rsync for reliability). Exclude local deploy logs.
# printf 'Transferring files to %s:%s ...\n' "$REMOTE_HOST" "$REMOTE_APP_DIR" | tee -a "$LOGFILE"
# if ! rsync -az --delete --exclude 'deploy_*.log' -e "ssh -i \"${SSH_KEY}\" -o StrictHostKeyChecking=no" \
#   "$LOCAL_BASE_DIR/" "${REMOTE_USER}@${REMOTE_HOST}:${REMOTE_APP_DIR}/"; then
#   printf 'File transfer failed\n' | tee -a "$LOGFILE"
#   exit $E_TRANSFER_FAIL
# fi

# printf 'Files transferred successfully to remote directory: %s\n' "$REMOTE_APP_DIR" | tee -a "$LOGFILE"
# log "Transferred repository contents to remote server at ${REMOTE_APP_DIR}"

# # === Step 5/6: Build / run app on remote host, configure nginx reverse proxy, validate ===
...
# ssh -i "$SSH_KEY" -o StrictHostKeyChecking=no -o ServerAliveInterval=60 -o ServerAliveCountMax=3 \
#   "${REMOTE_USER}@${REMOTE_HOST}" \
#   "mkdir -p \"${REMOTE_APP_DIR}\" && cat > \"${REMOTE_APP_DIR}/deploy_remote.sh\" && chmod +x \"${REMOTE_APP_DIR}/deploy_remote.sh\" && nohup sh \"${REMOTE_APP_DIR}/deploy_remote.sh\" > \"${REMOTE_APP_DIR}/deploy_remote.log\" 2>&1 &" <<EOF
# set -e
...
# # write .env for compose or docker run
# printf 'VITE_API_URL=%s\n' "\$VITE_API_URL" > .env || true

# # If docker-compose present, up will read .env automatically
...
#     sudo docker run -d --name "\$APP_NAME" --restart=always -p 127.0.0.1:\$APP_PORT:\$CONTAINER_PORT -e VITE_API_URL="\$VITE_API_URL" "\$APP_NAME:latest"
#   else
#     docker build -t "\$APP_NAME:latest" .
#     docker rm -f "\$APP_NAME" >/dev/null 2>&1 || true
#     docker run -d --name "\$APP_NAME" --restart=always -p 127.0.0.1:\$APP_PORT:\$CONTAINER_PORT -e VITE_API_URL="\$VITE_API_URL" "\$APP_NAME:latest"
#   fi
# fi

# # nginx config and validation steps...
...
# if sudo -n true 2>/dev/null; then
#   printf "%s\n" "${NGINX_BLOCK}" | sudo tee "${NGINX_CONF}" > /dev/null
#   sudo ln -sf "${NGINX_CONF}" "${NGINX_ENABLED}"
#   if sudo nginx -t >/dev/null 2>&1; then
#     sudo systemctl reload nginx || sudo nginx -s reload || true
#     echo "nginx: reloaded"
#   else
#     echo "nginx test failed; leaving config in place for manual inspection" >&2
#   fi
# else
#   echo "Warning: passwordless sudo not available; cannot write nginx config or reload automatically" >&2
#   echo "Manual steps (on remote):" >&2
#   echo "  printf '%s' \"${NGINX_BLOCK}\" | sudo tee ${NGINX_CONF} > /dev/null" >&2
#   echo "  sudo ln -sf ${NGINX_CONF} ${NGINX_ENABLED} && sudo nginx -t && sudo systemctl reload nginx" >&2
# fi

# # Basic validation: docker running, container status, and HTTP checks
# if sudo -n true 2>/dev/null; then
...
# docker ps --filter "name=${APP_NAME}" --format '  {{.Names}}\t{{.Status}}' || true

# # Health checks (proxy + direct container)
# # proxy check (port 80)
# if curl -fsS --max-time 5 "http://127.0.0.1/" >/dev/null 2>&1; then
#   echo "Proxy HTTP check: OK"
# else
#   echo "Proxy HTTP check: FAILED"
# fi

# # direct container check (container published port)
# if curl -fsS --max-time 5 "http://127.0.0.1:${APP_PORT}/" >/dev/null 2>&1; then
#   echo "Container HTTP check: OK"
# else
#   echo "Container HTTP check: FAILED"
# fi
# EOF
# printf 'Remote deployment launched (detached). Check remote log: %s:%s/deploy_remote.log\n' "$REMOTE_HOST" "$REMOTE_APP_DIR" | tee -a "$LOGFILE"
